...
namespace everisapi.API.Services
{
    public class AsignacionInfoRepository : IAsignacionInfoRepository
    {
        ...
        public IEnumerable<AsignacionSinPreguntasDto> GetAsignaciones()
        {
            ...
            var asignacionesTraducidas = _context.TraduccionesAsignaciones.Where(x => x.IdiomaId == 1).ToList();

            foreach (var t in asignacionesTraducidas)
            {
                AsignacionSinPreguntasDto traduccion = null;
                traduccion.Id = t.AsignacionesId;
                traduccion.Nombre = t.Traduccion;
                asignaciones.Add(traduccion);
            }
            asignaciones = asignaciones.OrderBy(t => t.Nombre).ToList();
            return asignaciones;
        }
        ...
        public List<PreguntaWithOneRespuestasDto> ChangePregunta(IEnumerable<PreguntaEntity> Preguntas, IEnumerable<RespuestaDto> Respuestas, int codigoIdioma)
        {
            ...
            foreach (var pregunta in Preguntas)
            {
                ...
                var PreguntaAdd = new PreguntaWithOneRespuestasDto
                {
                    Id = pregunta.Id,
                    Pregunta = traduccion.Traduccion,
                    Correcta = pregunta.Correcta,
                    Respuesta = RespuestaParaPregunta,
                    EsHabilitante = pregunta.EsHabilitante,
                    PreguntaHabilitanteId = pregunta.PreguntaHabilitanteId
                };
